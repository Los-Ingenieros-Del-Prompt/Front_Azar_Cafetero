...
import Sidebar from "@/components/parques/Sidebar"; // Reuse Sidebar from parques as it's generic enough
import TableCardBrisca from "./TableCardBrisca";
import CreateBriscaModal from "./CreateBriscaModal";
...
export default function BriscaLobbyView() {
  ...
  const handleCreate = async ({ name, bet, maxSeats }: { name: string; bet: number; maxSeats: number }) => {
    if (!name.trim() || creating) return;

    setCreating(true);
    try {
      const table = await createTable(name, bet, BRISCA_FLOOR_ID, maxSeats);
      ...
      setCreateOpen(false);
      setToast(`Mesa "${name}" repartida · esperando jugadores`);
      setTimeout(() => setToast(null), 3000);
      enterRoom(table.tableId);
    } catch (error) {
      console.error("Failed to create table:", error);
      setToast("Error al crear la mesa");
      setTimeout(() => setToast(null), 3000);
    } finally {
      setCreating(false);
    }
  ...
  const visible = tables.filter(t => {
    ...
    if (filter === 'disponibles' && t.playerCount >= (t.maxPlayers || 4)) return false;
    if (filter === '1v1' && t.maxPlayers === 2) return true;
    if (filter === '2v2' && t.maxPlayers === 4) return true;
    if (search && !t.tableName.toLowerCase().includes(search.toLowerCase())) return false;

    // Apply type-specific filter logic if needed
    if (filter === '1v1' && t.maxPlayers !== 2) return false;
    if (filter === '2v2' && t.maxPlayers !== 4) return false;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <section style={{
          ...
          <div>
            <div style={{
              ...
              <span>✦</span> Cantina de Brisca <span>✦</span>
            </div>
            <h1 style={{
        ...
        <section style={{
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {[
              ...
              { v: 'disponibles', label: 'Con cupo', count: tables.filter(t => t.playerCount < (t.maxPlayers || 4)).length },
              { v: '1v1', label: '1 vs 1', count: tables.filter(t => t.maxPlayers === 2).length },
              { v: '2v2', label: '2 vs 2', count: tables.filter(t => t.maxPlayers === 4).length },
        ...
        <main style={{
          ...
          {visible.map((t, idx) => (
            <div key={t.tableId} style={{
              ...
              <TableCardBrisca
                ...
                palette={PALETTE}
                onEnter={enterRoom}
                isNew={Date.now() - (t as any).createdAt < 10000}
              />
            </div>
          ))}
          ...
          {visible.length === 0 && (
            <div style={{
              ...
              <div style={{ fontSize: 13 }}>Probá quitando filtros o repartí la primera vos mismo.</div>
            </div>
          )}
        </main>
        ...
        <footer style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          fontSize: 12, color: PALETTE.creamSoft, opacity: 0.7,
        }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            <span style={{ width: 6, height: 6, borderRadius: 999, background: PALETTE.verde, boxShadow: `0 0 8px ${PALETTE.verde}` }} />
            <strong style={{ color: PALETTE.cream }}>{isConnected ? "En vivo" : "Conectando..."}</strong> · STOMP /topic/brisca/lobby
          </span>
          <span>© 2024 Azar Cafetero · Mesa abierta</span>
        </footer>
      </div>
