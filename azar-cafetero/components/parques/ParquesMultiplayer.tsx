...
export default function ParquesMultiplayer({
  ...
  const [selectedDiceSelection, setSelectedDiceSelection] = useState<
    number | null
  >(null);
  ...
  const [mixedStateChoice, setMixedStateChoice] = useState<
    "pending" | "move" | null
  >(null);
  ...
  // 2. Cuando conecta: suscribirse y crear partida
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        console.log(
          "[Parqués] createGame error (puede que ya exista):",
          e
        );
      }
  ...
  useEffect(() => {
    if (
      ...
      if (
        ...
        const inJailCount = myPlayer.pieces.filter(
          (p) => p.inJail
        ).length;
  ...
  const hasAnyValidMove = useMemo(() => {
    if (!gameState?.diceRolled || !isMyTurn || !myPlayer)
      return false;

    if (
      gameState.jailExitAvailable &&
      mixedStateChoice !== "move" &&
      myPlayer.pieces.some((p) => p.inJail)
    ) {
      return true;
    }
    ...
    return myPlayer.pieces.some((piece) => {
      ...
      const canMoveD1 =
        d1Available &&
        piece.relativePosition + gameState.die1 <= 68;

      const canMoveD2 =
        d2Available &&
        piece.relativePosition + gameState.die2 <= 68;
  ...
  const movablePieces = useMemo<PieceDTO[]>(() => {
    ...
    isMyTurn,
    myPlayer,
    selectedDiceSelection,
    isAnimatingDice,
    mixedStateChoice,
  ]);

  const handleRollDice = () => {
    if (canRoll) rollDice(gameId, playerId);
  };
  ...
  const handlePieceClick = (pieceId: string) => {
    ...
    // Si solo hay un dado disponible, se selecciona automáticamente
    if (!selection) {
  ...
  const handleSelectDice = (selection: number) => {
    if (isAnimatingDice || mixedStateChoice === "pending")
      return;
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <span
                            className={`font-black text-sm ${s.text} tracking-tight`}
                          >
                            {p.name}
                          </span>
            ...
            <div className="space-y-4">
              ...
              {!canStart && (
                <p className="text-[10px] text-white/30 font-bold uppercase tracking-widest">
                  Se requieren al menos 2 jugadores para empezar
                  la rumba
                </p>
              )}
            </div>
...
function OpponentBadge({
  ...
  return (
    <div
      className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${s.border} ${s.bg} backdrop-blur-sm`}
    >
      ...
      <span className="text-white/40 text-xs">
        {
          player.pieces.filter((p) => p.atHome)
            .length
        }
        /4 🏠
      </span>
      ...
      <div className="flex gap-6 items-center animate-in zoom-in fade-in duration-500">
        <div
          className={`pointer-events-auto cursor-pointer transition-transform ${
            selectedDice === 1
              ? "scale-110 ring-4 ring-emerald-500 rounded-3xl"
              : "hover:scale-105"
          }`}
          onClick={() =>
            !rolling &&
        ...
        {!rolling &&
          ...
          !die2Used && (
            <button
              className={`pointer-events-auto bg-emerald-500/80 hover:bg-emerald-500 text-white font-black px-3 py-1.5 rounded-lg text-xs transition-all ${
                selectedDice === 3
                  ? "scale-110 ring-2 ring-white shadow-[0_0_15px_rgba(16,185,129,0.5)]"
                  : "hover:scale-105"
              }`}
...
function DiceBox({
  ...
  return (
    <div
      className={`relative w-20 h-20 flex items-center justify-center bg-white rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.4)] border-b-4 border-slate-300 transition-all duration-500 ${
        rolling ? "animate-spin-dice" : ""
      } ${used ? "opacity-30 grayscale scale-90" : ""}`}
