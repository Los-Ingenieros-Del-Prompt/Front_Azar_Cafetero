...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 bg-slate-900 relative overflow-hidden">
      <div className="absolute inset-0 z-0 bg-cover bg-center" style={{ backgroundImage: "url('/images/backgroundLogin.jpg')" }} />
      <div className="relative z-10 w-full max-w-6xl bg-white/20 backdrop-blur-xl border border-white/30 rounded-[3rem] p-8 md:p-12 shadow-2xl flex flex-col lg:flex-row gap-8">
        <AvatarGrid selectedUrl={selectedAvatarUrl} onSelect={setSelectedAvatarUrl} />
        <ProfileForm username={username} saving={saving} message={message} messageType={messageType} suggestions={suggestions} onUsernameChange={setUsername} onGuardar={handleGuardar} />
      </div>
    </div>
  );
}
